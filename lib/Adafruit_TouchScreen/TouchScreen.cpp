...
#if (NUMSAMPLES > 2)
static int insert_sort(int array[], uint8_t size) {
  uint8_t j;
  int save;
  int valid_size = 0;
  ...
  for (int i = 1; i < size; i++) {
    save = array[i];
    if (save == 0 || save == 1023) {
      save = 1024; // discard invalid samples
    } else {
      valid_size++;
    }
...
TSPoint TouchScreen::getPoint(void) {
  ...
  *xm_port &= ~xm_pin;
...
#if NUMSAMPLES > 2
  valid_size = insert_sort(samples, NUMSAMPLES);
#endif
#if NUMSAMPLES == 2
  ...
  x = (1023 - samples[valid_size / 2]);
  //Serial.print("X raw: "); Serial.println(x);
  ...
  *yp_port |= yp_pin;
  ...
  y = (1023 - samples[valid_size / 2]);
  //Serial.print("Y raw: "); Serial.println(y);
  // Set X+ to ground
  // Set Y- to VCC
  // Hi-Z X- and Y+
  pinMode(_xp, OUTPUT);
